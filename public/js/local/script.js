$(document).ready(function () {
    ...
    // Import Locals - Added function
    $(document).on('click', '#BtnImportLocal', function(e) {
        e.preventDefault();
        console.log('Import button clicked'); // Debug message
        
        let formData = new FormData($('#FormImportLocal')[0]);
        formData.append('_token', csrf_token);
        ...
        $('#BtnImportLocal').prop('disabled', true).text('Importation...');
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                $('#BtnImportLocal').prop('disabled', false).text('Importer');
                console.log('Import success:', response); // Debug response
                
                if(response.status == 200) {
                    new AWN().success(response.message, {durations: {success: 5000}});
                    $('#ModalImportLocal').modal('hide');
                    $('.TableLocals').DataTable().ajax.reload();
                    $('#FormImportLocal')[0].reset();
                } else if(response.status == 400) {
                    $('.validationImportLocal').html("");
                    $('.validationImportLocal').addClass('alert alert-danger');
                    $.each(response.errors, function(key, list_err) {
                        $('.validationImportLocal').append('<li>' + list_err + '</li>');
                    });
                    
                    setTimeout(() => {
                        $('.validationImportLocal').fadeOut('slow', function() {
                            $(this).html("").removeClass('alert alert-danger').show();
                        });
                    }, 5000);
                } else if (response.status == 500) {
