$(document).ready(function () {
        ...
        function initializeDataTable(selector, url) {
            var tableFournisseurs = $(selector).DataTable({
                ...
                columns: [
                    ...
                    { data: 'siege_social', name: 'siege_social' },
                    { data: 'IF', name: 'IF' },
                    { data: 'CNSS', name: 'CNSS' },
            ...
            // Edit Fournisseur Event Handler
            $(selector + ' tbody').on('click', '.editFournisseur', function(e)
            {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#siege_social').val(response.siege_social || '');
                        $('#RC').val(response.RC || '');
                        $('#Patente').val(response.Patente || '');
                        $('#IF').val(response.IF || '');
                        $('#CNSS').val(response.CNSS || '');
            ...
            // Delete Fournisseur Event Handler
        ...
        $.ajax({
            ...
            success: function (response) 
            {
                ...
                else if(response.status == 400)
                {
                    ...
                    // Handling case where a supplier already exists
            ...
            error: function(xhr) {
                ...
                // Get custom error message from server
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    new AWN().alert(xhr.responseJSON.message, { durations: { alert: 5000 } });
                } else {
                    // Fallback to generic message only if no custom message is available
                    new AWN().alert("Une erreur est survenue, veuillez réessayer.", { durations: { alert: 5000 } });
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status == 200) {
                    ...
                    // Handling case where a supplier with the same name already exists
                    new AWN().alert(response.message, { durations: { alert: 5000 } });
                }
    ...
    // Import Fournisseur
    $('#BtnImportFournisseur').on('click', function(e) {
        e.preventDefault();
        
        let formData = new FormData($('#FormImportFournisseur')[0]);
        formData.append('_token', csrf_token);
        ...
        $('#BtnImportFournisseur').prop('disabled', true).text('Importation...');

        $.ajax({
            type: "POST",
            url: ImportFournisseur,
            data: formData,
            processData: false,
            contentType: false,
            dataType: "json",
            success: function (response) {
                $('#BtnImportFournisseur').prop('disabled', false).text('Importer');
                
                if(response.status == 200) {
                    new AWN().success(response.message, {durations: {success: 5000}});
                    $('#ModalImportFournisseur').modal('hide');
                    $('.TableFournisseurs').DataTable().ajax.reload();
                    $('#FormImportFournisseur')[0].reset();
                } else if(response.status == 400) {
                    $('.validationImportFournisseur').html("");
                    $('.validationImportFournisseur').addClass('alert alert-danger');
                    $.each(response.errors, function(key, list_err) {
                        $('.validationImportFournisseur').append('<li>' + list_err + '</li>');
                    });
                    
                    setTimeout(() => {
                        $('.validationImportFournisseur').fadeOut('slow', function() {
                            $(this).html("").removeClass('alert alert-danger').show();
                        });
                    }, 5000);
                } else if (response.status == 500) {
