$(document).ready(function () {
    ...
    $(document).on('click', '#BtnImportSubCategory', function(e) {
        e.preventDefault();
        
        let formData = new FormData($('#FormImportSubCategory')[0]);
        formData.append('_token', csrf_token);
        ...
        $('#BtnImportSubCategory').prop('disabled', true).text('Importation...');
    
        $.ajax({
            type: "POST",
            url: ImportSubCategory,
            data: formData,
            processData: false,
            contentType: false,
            dataType: "json",
            success: function (response) {
                $('#BtnImportSubCategory').prop('disabled', false).text('Importer');
                
                if(response.status == 200) {
                    new AWN().success(response.message, {durations: {success: 5000}});
                    $('#ModalImportSubCategory').modal('hide');
                    $('.TableSubCategories').DataTable().ajax.reload();
                    $('#FormImportSubCategory')[0].reset();
                } else if(response.status == 400) {
                    $('.validationImportSubCategory').html("");
                    $('.validationImportSubCategory').addClass('alert alert-danger');
                    $.each(response.errors, function(key, list_err) {
                        $('.validationImportSubCategory').append('<li>' + list_err + '</li>');
                    });
                    
                    setTimeout(() => {
                        $('.validationImportSubCategory').fadeOut('slow', function() {
                            $(this).html("").removeClass('alert alert-danger').show();
                        });
                    }, 5000);
                } else if (response.status == 500) {
