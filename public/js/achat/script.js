$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.status == 200) {
                    ...
                    if ($.fn.DataTable.isDataTable('.TableProductAchat')) {
                        $('.TableProductAchat').DataTable().clear().draw();
                    }
                    
                    // Clear temporary items table
                    if ($.fn.DataTable.isDataTable('.TableAmpAchat')) {
                        $('.TableAmpAchat').DataTable().clear().draw();
                    }
                    
                    // Reset the total display
                    $('.TotalByFournisseurAndUser').text("0.00 DH");
    ...
    $('.TableAchat tbody').on('click', '.bg-primary-subtle', function(e) {
        ...
        $('#BtnChangeStatusAchat').attr('data-id',$(this).attr('data-id'));
  ...
  $('#BtnUpdateAchat').on('click', function(e) {
    ...
    $('#BtnChangeStatusAchat').off('click').on('click',function(e) 
    {
        e.preventDefault();
        let status = $('#StatusAchat').val();
        let IdAchat= $(this).attr('data-id');
        if(status == 0){
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                {
                    $('#ModalEditAchat').modal("hide");
                    new AWN().success(response.message, { durations: { alert: 5000 } });
                    initializeTableAchatDataTable();
...
$('#BtnAddProduct').on('click', function(e) {
    ...
    console.log('Save product button clicked'); // For debugging
    ...
    $.ajax({
        type: "POST",
        url: AddProduct, // Using the variable from your blade template
        data: formData,
