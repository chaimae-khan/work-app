$(document).ready(function () {
    ...
    function initializeDataTable() {
        try {
            ...
            var tableCategories = $('.TableCategories').DataTable({
                ...
                columns: [
                    { data: 'name', name: 'name' },
                    { data: 'classe', name: 'classe' }, // Added classe column
            ...
            $('.TableCategories tbody').on('click', '.editCategory', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#classe').val(response.classe); // Added classe field
    ...
    $('#BtnUpdateCategory').on('click', function(e) {
        ...
        formData.append('classe', $('#classe').val()); // Added classe field
    ...
    $(document).on('click', '#BtnImportCategory', function(e) {
        ...
        let formData = new FormData($('#FormImportCategory')[0]);
        formData.append('_token', csrf_token);
        ...
        $('#BtnImportCategory').prop('disabled', true).text('Importation...');
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                $('#BtnImportCategory').prop('disabled', false).text('Importer');
                
                if(response.status == 200) {
                    new AWN().success(response.message, {durations: {success: 5000}});
                    $('#ModalImportCategory').modal('hide');
                    $('.TableCategories').DataTable().ajax.reload();
                    $('#FormImportCategory')[0].reset();
                } else if(response.status == 400) {
                    $('.validationImportCategory').html("");
                    $('.validationImportCategory').addClass('alert alert-danger');
                    $.each(response.errors, function(key, list_err) {
                        $('.validationImportCategory').append('<li>' + list_err + '</li>');
                    });
                    
                    setTimeout(() => {
                        $('.validationImportCategory').fadeOut('slow', function() {
                            $(this).html("").removeClass('alert alert-danger').show();
                        });
                    }, 5000);
                } else if (response.status == 500) {
