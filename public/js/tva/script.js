$(document).ready(function () {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.message) {
                    // Si c'est une erreur de validation (duplicate), afficher comme warning
    ...
    // Import TVA
$(document).on('click', '#BtnImportTva', function(e) {
    e.preventDefault();
    
    let formData = new FormData($('#FormImportTva')[0]);
    formData.append('_token', csrf_token);
    ...
    $('#BtnImportTva').prop('disabled', true).text('Importation...');
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            $('#BtnImportTva').prop('disabled', false).text('Importer');
            
            if(response.status == 200) {
                new AWN().success(response.message, {durations: {success: 5000}});
                $('#ModalImportTva').modal('hide');
                $('.TableTvas').DataTable().ajax.reload();
                $('#FormImportTva')[0].reset();
            } else if(response.status == 400) {
                $('.validationImportTva').html("");
                $('.validationImportTva').addClass('alert alert-danger');
                $.each(response.errors, function(key, list_err) {
                    $('.validationImportTva').append('<li>' + list_err + '</li>');
                });
                
                setTimeout(() => {
                    $('.validationImportTva').fadeOut('slow', function() {
                        $(this).html("").removeClass('alert alert-danger').show();
                    });
                }, 5000);
            } else if (response.status == 500) {
